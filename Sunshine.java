...
Programa para determinar parámetros solares.
    ...
    beta   : �?ngulo con respecto al norte, vector
*/
//Declaración de variables

        double [][] delta = new double [365][24];
        double [][] delt = new double [365][24];
	int h, N, i, j, k, l;
	...
	double phi, hh, li, cont1, cont2, pi;
...
//}	//eliminar esta linea cuando le code sirva
...
//Declaración de constantes *provisional* (se le tienen que pedir al usuario)
	...
	for (N=0; N<364; N++)
                ...
                {
                for (h=0; h<23; h++)
                x[N][h] = Math.toRadians((2*pi/365)*(N-1+((h-12)/24)));

                delt[N][h] = (0.006918-0.399912*Math.cos(x[N][h])+0.070257*Math.sin(x[N][h])-0.006758*Math.cos(2*x[N][h])+0.000907*Math.sin(2*x[N][h])-0.002697*Math.cos(3*x[N][h])+0.001480*Math.sin(3*x[N][h]));

                delta[N][h] = delt[N][h]*(180/pi);
	...
	for (N=0; N<364; N++)
                {
                for (h=0; h<23; h++)
                System.out.println(0.006918-0.399912*Math.cos(x[N][h])+0.070257*Math.sin(x[N][h])-0.006758*Math.cos(2*x[N][h])+0.000907*Math.sin(2*x[N][h])-0.002697*Math.cos(3*x[N][h])+0.001480*Math.sin(3*x[N][h]));
                }
        }
        ...
        double [] beta;
        beta = new double [181];	//pinshi declaración salida de tono alv
        ...
        for (k=0; k<180; k++)
                ...
                //System.out.println(azimut[N][l]);
                }
        }

        //{
        //for (j=0; j<181; j++)
        //beta[j] = Math.toDegrees(beta[j]);
        //}
